...
router.get("/me", customerAuth, async (req: CustomerAuthRequest, res) => {
...
router.patch("/me", customerAuth, async (req: CustomerAuthRequest, res) => {
  ...
  const customer = await Customer.findByIdAndUpdate(req.customer!.id, update, { new: true }).select(
    "-password"
  );
  res.json({
    id: customer!.id,
