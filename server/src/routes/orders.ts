...
router.get("/", auth, async (req: AuthRequest, res) => {
  ...
  if (role === "chef") filter.status = { $in: ["pending", "preparing", "ready"] };
  if (role === "waiter") {
    filter.type = "dine_in";
...
router.patch("/:id/status", auth, async (req: AuthRequest, res) => {
  ...
  if (status === "served" && order.tableId) {
    ...
    await Table.findByIdAndUpdate(tid, { status: "available" });
  }

  emitOrderUpdate(serialized);
  ...
  if (status === "ready") {
    if (order.type === "delivery") {
      ...
      emitNotification("waiter", { type: "order_ready", order: serialized });
    }
  }
  res.json(serialized);
});
...
router.post("/:id/call-waiter", async (req, res) => {
  ...
  if (!order) return res.status(404).json({ message: "Not found" });
  const serialized = serializeOrder(order);
  emitNotification("waiter", { type: "call_waiter", order: serialized });
...
router.post("/:id/request-bill", async (req, res) => {
  ...
  if (order.tableId) {
    ...
    emitNotification("waiter", { type: "request_bill", order: serialized });
