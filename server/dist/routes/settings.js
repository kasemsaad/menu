...
router.patch("/", auth, requireRole("admin"), async (req, res) => {
    const { taxPercent, servicePercent, deliveryFee, whatsappNumber, restaurantName, logo, primaryColor, accentColor, openTime, closeTime, notificationSounds, soundVolume, } = req.body;
    let settings = await Settings.findOne();
    if (!settings)
        settings = await Settings.create({});
    ...
    if (servicePercent !== undefined)
        settings.servicePercent = servicePercent;
    ...
    if (notificationSounds !== undefined)
        settings.notificationSounds = notificationSounds;
