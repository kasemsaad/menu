...
router.patch("/:id", auth, requireRole("admin"), async (req, res) => {
    ...
    if (tableNumber !== undefined)
        update.tableNumber = tableNumber === "" ? undefined : Number(tableNumber);
    const review = await Review.findByIdAndUpdate(req.params.id, update, { new: true }).populate("productId", "name");
    if (!review)
