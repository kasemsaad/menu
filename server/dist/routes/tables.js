...
router.get("/", auth, requireRole("admin", "waiter"), async (req, res) => {
    const filter = {};
...
router.patch("/:id/status", auth, requireRole("admin", "waiter"), async (req, res) => {
    if (req.user?.role === "waiter") {
        const waiter = await User.findById(req.user.id);
        if (!waiter?.onShift)
            return res.status(403).json({ message: "Start your shift first" });
        const allowed = waiter.assignedTableIds?.some((id) => id.toString() === req.params.id);
        if (!allowed)
            return res.status(403).json({ message: "Table not in your assignment" });
    }
...
router.get("/:id/check", auth, requireRole("admin", "waiter"), async (req, res) => {
    if (req.user?.role === "waiter") {
        const waiter = await User.findById(req.user.id);
        if (!waiter?.onShift)
            return res.status(403).json({ message: "Start your shift first" });
...
router.post("/:id/reset", auth, requireRole("admin", "waiter"), async (req, res) => {
    if (req.user?.role === "waiter") {
        const waiter = await User.findById(req.user.id);
        if (!waiter?.onShift)
            return res.status(403).json({ message: "Start your shift first" });
    ...
    const table = await Table.findByIdAndUpdate(req.params.id, { status: "available" }, { new: true });
    if (!table)
        return res.status(404).json({ message: "Not found" });
    const now = new Date();
    await Order.updateMany({ tableId: req.params.id, type: "dine_in", ...activeOrderFilter }, { deletedAt: now, callWaiter: false });
