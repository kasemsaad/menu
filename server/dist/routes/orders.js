...
const calcTotals = async (items, couponCode, type) => {
...
router.post("/", optionalCustomerAuth, async (req, res) => {
    const { tableId, items, type = "dine_in", paymentMethod = "cash", couponCode, customerName, customerPhone, deliveryAddress, deliveryLat, deliveryLng, notes, } = req.body;
    let tableNumber;
    if (tableId) {
...
router.get("/", auth, async (req, res) => {
    ...
    if (role === "waiter") {
        ...
        if (!waiter?.onShift || !waiter.assignedTableIds?.length)
            return res.json([]);
        filter.type = "dine_in";
...
router.patch("/:id/status", auth, async (req, res) => {
    ...
    const order = await Order.findOneAndUpdate({ _id: req.params.id, ...activeOrderFilter }, update, { new: true }).populate("tableId", "number");
    if (!order)
        return res.status(404).json({ message: "Not found" });
...
router.post("/:id/call-waiter", async (req, res) => {
    const order = await Order.findOneAndUpdate({ _id: req.params.id, ...activeOrderFilter }, { callWaiter: true }, { new: true }).populate("tableId", "number");
    if (!order)
        return res.status(404).json({ message: "Not found" });
    const serialized = serializeOrder(order);
    await emitWaiterTableNotification(tableIdFromOrder(serialized), {
