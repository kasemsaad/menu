...
router.post("/login", async (req, res) => {
    ...
    const token = jwt.sign({ id: user.id, role: user.role, branchId: user.branchId }, process.env.JWT_SECRET || "secret", { expiresIn: "7d" });
    res.json({
        token,
...
router.post("/shift/end", auth, async (req, res) => {
    const user = await User.findById(req.user.id);
    if (!user || !shiftRoles.includes(user.role)) {
        return res.status(403).json({ message: "Shift not available for this role" });
    }
...
router.patch("/shift/tables", auth, async (req, res) => {
    const user = await User.findById(req.user.id);
