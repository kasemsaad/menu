...
export const tableIdFromOrder = (order) => {
    const o = order;
    const tid = o.tableId;
    if (tid == null)
        return null;
...
export const notifyWaitersForTable = async (tableId, payload) => {
    const io = getIO();
    if (!io || !tableId || !mongoose.isValidObjectId(tableId))
        return;
...
export const emitOrderUpdatedToStaff = async (order) => {
    const io = getIO();
    if (!io)
        return;
    ...
    const o = order;
    if (o._id)
        io.to(`order:${o._id}`).emit("order:updated", order);
};
export const emitNewOrderToStaff = async (order) => {
    const io = getIO();
    if (!io)
        return;
    const o = order;
    ...
    if (o.type === "delivery") {
        ...
        if (tid)
            io.to(`table:${tid}`).emit("order:new", order);
    }
};
