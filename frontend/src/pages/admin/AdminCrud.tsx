...
export const AdminCrud = ({ resource }: { resource: Resource }) => {
  ...
  const load = async (pageNumber = 1) => {
    setLoading(true);
    try {
      const response = await api.get(`${endpoints[resource]}?page=${pageNumber}&perpage=${perpage}`);
  ...
  const generateTable = async () => {
    const num = total != null ? total + 1 : items.length + 1;
    ...
    <section className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <p className="text-sm text-stone-500">
          {t("page")} {page} / {Math.max(1, Math.ceil((total ?? 0) / perpage))} · {total ?? 0} {t("items")}
        </p>
        <button
          className="btn-ghost"
          type="button"
          disabled={(total ?? 0) <= page * perpage}
